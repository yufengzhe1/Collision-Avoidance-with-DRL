        ...
        for agent_idx in range(2):
            ...
            if phase == 'train' and probability < epsilon:
                ...
                for action in action_space:
                    ...
                    value_temp1 = pow(gamma, state.v_pref)
                    value_temp2 = model(sn_est, device)
                    value_temp3 = value_temp2.data
                    value_temp4 = value_temp3.item()
                    value = reward + value_temp1 * value_temp4
                    if value > max_value:
                        max_value = value
                        best_action = action
                action = best_action
    ...
    for traj_file in os.listdir(traj_dir):
        ...
        with open(os.path.join(traj_dir, traj_file)) as fo:
            ...
            for line in lines[2:]:
                ...
                t3=(t1,t2)
                positions.append(t3)
                # positions=[[float(x) for x in re.sub('[\']', '', po).split(',')] for po in positions]
                #print(positions)
